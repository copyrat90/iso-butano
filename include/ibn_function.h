...
//       (See accompanying file `licenses/function2.txt` or copy at
//             http://www.boost.org/LICENSE_1_0.txt)

#ifndef FU2_INCLUDED_FUNCTION2_HPP_
#define FU2_INCLUDED_FUNCTION2_HPP_

// Edit by copyrat90: Use `BN_ASSERT` instead
#include <bn_assert.h>
...
// Edit by copyrat90: Silence the warnings
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wswitch-default"

// Edit by copyrat90: Exceptions are disabled
...
#ifndef NDEBUG
// NOLINTNEXTLINE(cppcoreguidelines-macro-usage)
#define FU2_DETAIL_UNREACHABLE() ::ibn::detail::unreachable_debug()
#else
// NOLINTNEXTLINE(cppcoreguidelines-macro-usage)
#define FU2_DETAIL_UNREACHABLE() FU2_DETAIL_UNREACHABLE_INTRINSIC()
#endif

namespace ibn // Edit by copyrat90: Change namespace
{
...
struct is_noexcept_correct<true, T, identity<Args...>>
    : std::integral_constant<bool,
                             noexcept(::ibn::detail::invocation::invoke(std::declval<T>(), std::declval<Args>()...))>
{
};
...
struct overload_impl<Current, Next, Rest...> : Current, overload_impl<Next, Rest...>
{
    explicit overload_impl(Current current, Next next, Rest... rest)
        : Current(std::move(current)), overload_impl<Next, Rest...>(std::move(next), std::move(rest)...)
    {
    }
...
FU2_DETAIL_CXX14_CONSTEXPR auto retrieve(std::true_type /*is_inplace*/, Accessor from, std::size_t from_capacity)
{
    ...
    // Edit by copyrat90: Check `std::align()` failure
    auto aligned = std::align(alignof(T), sizeof(T), inplace, from_capacity);
    BN_ASSERT(aligned, "Align failed!");
    return type(aligned);
}
    ...
    struct trait
    {
                                ...
                                data_accessor* to, std::size_t to_capacity)
        {
            ...
            case opcode::op_move: {
                ...
                BN_ASSERT(box, "The object must not be over aligned or null!");
            ...
            case opcode::op_copy: {
                ...
                BN_ASSERT(std::is_copy_constructible<T>::value, "The box is required to be copyable here!");
            ...
            case opcode::op_weak_destroy: {
                ...
                BN_ASSERT(!to && !to_capacity, "Arg overflow!");
                auto box = static_cast<T*>(retrieve<T>(std::integral_constant<bool, IsInplace>{}, from, from_capacity));
...
public:
    ...
    FU2_DETAIL_CXX14_CONSTEXPR erasure(std::false_type /*use_bool_op*/, T&& callable,
    ...
    template <typename T, typename Allocator = std::allocator<std::decay_t<T>>>
    FU2_DETAIL_CXX14_CONSTEXPR erasure(std::true_type /*use_bool_op*/, T&& callable,
                                       Allocator&& allocator_ = Allocator{})
    {
        if (!!callable)
        {
    ...
    static constexpr decltype(auto) invoke(Erasure&& erasure, Args&&... args)
    {
        ...
        return erasure.vtable_.template invoke<Index>(std::forward<Erasure>(erasure).opaque_ptr(), capacity,
                                                      std::forward<Args>(args)...);
    }
};
...
public:
    ...
    constexpr erasure() noexcept
        : invoke_table_(invoke_table_t::template get_empty_invocation_table<IsThrowing>()), view_(nullptr)
    {
    }
...
/// `ibn::detail::function` types only.
template <typename T>
struct use_bool_op : std::false_type
{
};
...
using object_size = std::integral_constant<std::size_t, 16U>; // Edit by copyrat90: `32U`->`16U`
} // namespace detail
} // namespace abi_400
...
///
template <bool IsOwning, bool IsCopyable, typename Capacity, bool IsThrowing, bool HasStrongExceptGuarantee,
          typename... Signatures>
...
///
///   ```cpp
///   auto overloaded = ibn::overload([](std::true_type) { return true; },
...
struct uses_allocator<::ibn::detail::function<Config, Property>, Alloc> : std::true_type
{
};
} // namespace std
