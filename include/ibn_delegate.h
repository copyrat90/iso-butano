...
namespace ibn // (Edit) Add namespace
{
...
template <>
inline void nullReturnFunction()
{
    return;
}
...
public:
    ...
    union DataPtr {
        ...
        static constexpr bool equal(Trampoline fkn, const DataPtr& lhs, const DataPtr& rhs) noexcept
        {
            // Ugly, but the m_ptr part should be optimized away on normal
            // platforms.
            return (fkn == doRuntimeFkn ? (lhs.fkn_ptr == rhs.fkn_ptr) : (lhs.v_ptr == rhs.v_ptr));
        }
        static constexpr bool less(Trampoline fkn, const DataPtr& lhs, const DataPtr& rhs) noexcept
        {
            // Ugly, but the m_ptr part should be optimized away on normal
            // platforms.
            return fkn == doRuntimeFkn ? (lhs.fkn_ptr < rhs.fkn_ptr) : (lhs.v_ptr < rhs.v_ptr);
    ...
    struct FknStore
    {
        ...
        constexpr FknStore(Trampoline fkn, void* ptr) : m_fkn(fkn), m_data(ptr) {};
        constexpr FknStore(FknPtr ptr) : m_fkn(ptr ? &DataPtr::doRuntimeFkn : &doNullCB), m_data(ptr)
        {
        }
    ...
    static constexpr bool equal(const FknStore& lhs, const FknStore& rhs) noexcept
    {
        return lhs.m_fkn == rhs.m_fkn && DataPtr::equal(lhs.m_fkn, lhs.m_data, rhs.m_data);
    }

    static constexpr bool less(const FknStore& lhs, const FknStore& rhs) noexcept
    {
        return (lhs.null() && !rhs.null()) || lhs.m_fkn < rhs.m_fkn ||
               (lhs.m_fkn == rhs.m_fkn && DataPtr::less(lhs.m_fkn, lhs.m_data, rhs.m_data));
    }
    ...
    struct DeduceMemberType<R (T::*)(Args...), mf>
    {
        ...
        static constexpr Trampoline trampoline = &common::template doMemberCB<ObjType, mf>;
    ...
    struct DeduceMemberType<R (T::*)(Args...) const, mf>
    {
        ...
        static constexpr Trampoline trampoline = &common::template doConstMemberCB<ObjType, mf>;
...
public:
    ...
    static constexpr bool equal(const mem_fkn_base& lhs, const mem_fkn_base& rhs)
    {
...
public:
     ...
     * union.
     */
    constexpr delegate(Trampoline tFkn, void* datap) noexcept : m_data(tFkn, datap)
    {
    }
    ...
    static constexpr bool equal(const delegate& lhs, const delegate& rhs) noexcept
    {
    ...
    struct Equal
    {
        constexpr bool operator()(const delegate& lhs, const delegate& rhs) const noexcept
        {
    ...
    static constexpr bool less(const delegate& lhs, const delegate& rhs) noexcept
    {
    ...
    {
        m_data = FknStore(&common::template doMemberCB<T, memFkn>, static_cast<void*>(&tr));
        return *this;
    }
    ...
    {
        m_data =
            FknStore(&common::template doConstMemberCB<T, memFkn>, const_cast<void*>(static_cast<const void*>(&tr)));
        return *this;
    }
    ...
    DELEGATE_CXX14CONSTEXPR delegate& set(T const& tr) noexcept
    {
        m_data = FknStore(&doConstFunctor<T>, const_cast<void*>(static_cast<const void*>(&tr)));
        return *this;
    }
     ...
     */
    template <class T>
    DELEGATE_CXX14CONSTEXPR delegate& set(mem_fkn<T, false, R(Args...)> const& f, T& o) noexcept
    {
        m_data = FknStore(f.ptr(), static_cast<void*>(&o));
        return *this;
    }
    template <class T>
    DELEGATE_CXX14CONSTEXPR delegate& set(mem_fkn<T, false, R(Args...)> const& f, T const& o) = delete;

    template <class T>
    DELEGATE_CXX14CONSTEXPR delegate& set(mem_fkn<T, true, R(Args...)> const& f, T const& o) noexcept
    {
        m_data = FknStore(f.ptr(), const_cast<void*>(static_cast<const void*>(&o)));
        return *this;
    }
    template <class T>
    DELEGATE_CXX14CONSTEXPR delegate& set(mem_fkn<T, true, R(Args...)> const& f, T& o) noexcept
    {
        return set(f, static_cast<T const&>(o));
    }

    template <class T>
    DELEGATE_CXX14CONSTEXPR delegate& set(mem_fkn<T, false, R(Args...)> const& f, T&& o) = delete;
    template <class T>
    DELEGATE_CXX14CONSTEXPR delegate& set(mem_fkn<T, true, R(Args...)> const& f, T&& o) = delete;
    ...
    template <auto mFkn>
    constexpr delegate& set(typename common::template DeduceMemberType<decltype(mFkn), mFkn>::ObjType& obj) noexcept
    {
        using DM = typename common::template DeduceMemberType<decltype(mFkn), mFkn>;
        m_data = FknStore(DM::trampoline, DM::castPtr(&obj));
        return *this;
    }
    template <auto mFkn>
    constexpr delegate& set(
        typename common::template DeduceMemberType<decltype(mFkn), mFkn>::ObjType const& obj) noexcept
    {
        using DM = typename common::template DeduceMemberType<decltype(mFkn), mFkn>;
        m_data = FknStore(DM::trampoline, DM::castPtr(&obj));
        return *this;
    ...
    template <auto mFkn>
    constexpr delegate& set(typename common::template DeduceMemberType<decltype(mFkn), mFkn>::ObjType&& obj) = delete;
#endif
    ...
    DELEGATE_CXX14CONSTEXPR delegate& set_free_with_void(void const* ctx) noexcept
    {
        m_data = FknStore(&dofreeFknWithVoidConstPtr<fkn>, const_cast<void*>(ctx));
        return *this;
    }
    ...
    {
        m_data = FknStore(&dofreeFknWithObjectRef<T, fkn>, const_cast<void*>(static_cast<const void*>(&o)));
        return *this;
    }
    ...
    DELEGATE_CXX14CONSTEXPR delegate& set_free_with_object(T& o) noexcept
    {
        m_data = FknStore(&dofreeFknWithObjectConstRef<T, fkn>, static_cast<void*>(&o));
        return *this;
    }
    ...
    {
        m_data = FknStore(&dofreeFknWithObjectConstRef<T, fkn>, const_cast<void*>(static_cast<void const*>(&o)));
        return *this;
    }
    ...
    {
        return delegate{&common::template doMemberCB<T, memFkn>, static_cast<void*>(&o)};
    }

    template <class T, R (T::*memFkn)(Args... args) const>
    static constexpr delegate make(const T& o) noexcept
    {
        return delegate{&common::template doConstMemberCB<T, memFkn>, const_cast<void*>(static_cast<const void*>(&o))};
    }
    ...
    {
        return delegate{&doConstFunctor<T>, const_cast<void*>(static_cast<const void*>(&o))};
    }
    template <class T>
    ...
    {
        return delegate{&dofreeFknWithVoidConstPtr<fkn>, const_cast<void*>(ctx)};
    }
    ...
    {
        return delegate{&dofreeFknWithObjectRef<T, fkn>, static_cast<void*>(&o)};
    }

    template <typename T, R (*fkn)(T const&, Args...)>
    static constexpr delegate make_free_with_object(T& o) noexcept
    {
        return delegate{&dofreeFknWithObjectConstRef<T, fkn>, static_cast<void const*>(&o)};
    }

    template <typename T, R (*fkn)(T const&, Args...)>
    static constexpr delegate make_free_with_object(T const& o) noexcept
    {
        return delegate{&dofreeFknWithObjectConstRef<T, fkn>, const_cast<void*>(static_cast<void const*>(&o))};
    }
    ...
    template <class T>
    static constexpr delegate make(mem_fkn<T, false, R(Args...)> f, T& o) noexcept
    {
        return delegate{f.ptr(), static_cast<void*>(&o)};
    }
    template <class T>
    static constexpr delegate make(mem_fkn<T, false, R(Args...)>, T const&) = delete;

    template <class T>
    static constexpr delegate make(mem_fkn<T, true, R(Args...)> f, T const& o) noexcept
    {
        return delegate{f.ptr(), const_cast<void*>(static_cast<const void*>(&o))};
    }
    template <class T>
    static constexpr delegate make(mem_fkn<T, true, R(Args...)> f, T& o) noexcept
    {
        return delegate{f.ptr(), static_cast<void*>(&o)};
    }
    ...
    static constexpr delegate make(
        typename common::template DeduceMemberType<decltype(mFkn), mFkn>::ObjType& obj) noexcept
    {
        using DM = typename common::template DeduceMemberType<decltype(mFkn), mFkn>;
    ...
    template <auto mFkn>
    static constexpr delegate make(typename common::template DeduceMemberType<decltype(mFkn), mFkn>::ObjType&&) =
        delete;
...
template <typename R, typename... Args>
constexpr bool operator==(const delegate<R(Args...)>& lhs, const delegate<R(Args...)>& rhs) noexcept
{
...
template <typename R, typename... Args>
constexpr bool operator==(details::nullptr_t, const delegate<R(Args...)>& rhs) noexcept
{
    return rhs.null();
}

template <typename R, typename... Args>
constexpr bool operator!=(details::nullptr_t lhs, const delegate<R(Args...)>& rhs) noexcept
{
    return !(lhs == rhs);
}

template <typename R, typename... Args>
constexpr bool operator==(const delegate<R(Args...)>& lhs, details::nullptr_t) noexcept
{
    return lhs.null();
}

template <typename R, typename... Args>
constexpr bool operator!=(const delegate<R(Args...)>& lhs, details::nullptr_t rhs) noexcept
{
    return !(lhs == rhs);
}
...
#define DELEGATE_MKMEM(signature, memFknPtr, object) \
    (delegate<signature>::make<std::remove_reference_t<decltype(object)>, memFkn>(object))
